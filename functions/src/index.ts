...
function buildClosetContext(closetItems?: ClosetItem[]): string {
  if (!closetItems || closetItems.length === 0) return "";

  const grouped: Record<string, string[]> = {};
  closetItems.forEach(({ category, color, brand }) => {
    const key = category || "Other";
    const label = [color, brand].filter(Boolean).join(" ") + " " + key.toLowerCase();
    if (!grouped[key]) grouped[key] = [];
    grouped[key].push(label.trim());
  });
  ...
  return `User's wardrobe (${closetItems.length} item${closetItems.length !== 1 ? "s" : ""}):\n${lines}\nWhen suggesting outfits or giving styling advice, refer to these actual items the user owns.`.trim();
}

function buildColorProfileContext(colorProfile?: ColorProfile): string {
  if (!colorProfile) return "";
  const paletteNames = colorProfile.palette.map((c) => c.name).join(", ");
  const avoidNames = colorProfile.avoid.join(", ");
  const secondary = colorProfile.secondarySeason
    ? ` with secondary ${colorProfile.secondarySeason} tones`
    : "";
  return `
User color profile:
- Season type: ${colorProfile.season}${secondary}
- Best colors: ${paletteNames}
- Colors to avoid: ${avoidNames}
When giving outfit or color advice, naturally factor in this profile. Recommend their best colors and avoid suggesting colors from their avoid list.
`.trim();
}

function buildChatSystemPrompt(colorProfile?: ColorProfile, closetItems?: ClosetItem[]): string {
  const colorContext = buildColorProfileContext(colorProfile);
  const closetContext = buildClosetContext(closetItems);
  return `
...
function buildVisionSystemPrompt(colorProfile?: ColorProfile, closetItems?: ClosetItem[]): string {
  const colorContext = buildColorProfileContext(colorProfile);
  const closetContext = buildClosetContext(closetItems);
  return `
...
function buildOutfitSystemPrompt(colorProfile?: ColorProfile, closetItems?: ClosetItem[]): string {
  const colorContext = buildColorProfileContext(colorProfile);
  const closetContext = buildClosetContext(closetItems);
  return `
You are Aura, a premium conversational AI fashion companion.
...
${data.message}
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: buildVisionSystemPrompt(data.colorProfile, data.closetItems),
      },
      {
        role: "user",
        content,
...
`.trim();
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: buildOutfitSystemPrompt(data.colorProfile, data.closetItems),
        },
        {
          role: "user",
          content: userPrompt,
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: buildChatSystemPrompt(data.colorProfile, data.closetItems),
      },
      {
        role: "user",
        content: `
